...
server.route([
    {
        ...
        handler: function(request, reply) {
            User.findOne({"email": request.payload.email}, function(err, existing_user){
                if(existing_user){
                    reply("This email has already been registered, please try another email").code(400);
                } else {
                    var user = new User({"email": request.payload.email, "name": request.payload.name, "password": request.payload.password});
                    user.save(function(err, saved_user_service){
                        if(err){
                            reply("Error durign sign up").code(400);
                        } else {
